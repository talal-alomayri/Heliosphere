...
export default function AsteroidBeltPage() {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-start text-left px-12 md:px-24 max-w-4xl pt-30">
        <h1
        ...
        <p className="text-white/90 text-xl md:text-2xl leading-relaxed mb-10">
          ...
          {ceres && !isPending && (
            <>
              {" "}
              Its largest member,{" "}
              <strong className="text-cyan-300">Ceres</strong>, is a dwarf
              planet with a radius of{" "}
              {ceres.meanRadius ? `${ceres.meanRadius} km` : "N/A"} and an
              orbital period of{" "}
              {ceres.sideralOrbit ? `${ceres.sideralOrbit} days` : "N/A"}.
            </>
          )}
        </p>
          ...
          <div className="w-full mb-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                {
                  label: "Radius",
                  value: ceres.meanRadius ? ceres.meanRadius + " km" : "N/A",
                },
                {
                  label: "Gravity",
                  value: ceres.gravity ? ceres.gravity + " m/s²" : "N/A",
                },
                {
                  label: "Avg Temp",
                  value: ceres.avgTemp ? ceres.avgTemp + " K" : "N/A",
                },
                {
                  label: "Orbit",
                  value: ceres.sideralOrbit
                    ? ceres.sideralOrbit + " days"
                    : "N/A",
                },
              ].map(({ label, value }) => (
                <div
                  key={label}
                  className="bg-black/50 border border-cyan-400/30 backdrop-blur-sm px-4 py-3 rounded-lg"
                >
                  <span className="block text-cyan-400 text-xs font-bold uppercase tracking-wider mb-1">
                    {label}
                  </span>
        ...
        {asteroids.length > 0 && (
          <div className="w-full">
            <p className="text-cyan-400 text-xs font-bold tracking-widest uppercase mb-3">
              Notable Asteroids (sorted by orbit)
            </p>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {asteroids.map((a) => (
                <div
                  key={a.id}
                  className="bg-black/50 border border-white/10 backdrop-blur-sm px-4 py-3 rounded-lg"
                >
                  <span className="block text-white font-semibold text-sm mb-1">
                    {a.englishName || a.id}
                  </span>
                  <span className="text-white/60 text-xs">
                    {a.meanRadius ? a.meanRadius + " km radius" : "radius N/A"}
                  </span>
                </div>
              ))}
            </div>
        ...
        {isError && (
          <p className="text-red-400/70 text-sm mt-4">
            ⚠ API error — verify your token in .env
          </p>
        )}
      </div>
    </div>
