...
import bgImg from '../../assets/images/planet-mercury-info-pixel.webp';
import iconImg from '../../assets/images/planet-mercury-alone-pixel.webp';
import { useMercury } from '../../hooks/useSpaceData';
...
export default function MercuryPage() {
  ...
  const { data, isPending, isError } = useMercury();
  ...
  return (
    <div ref={containerRef} className="h-screen overflow-y-auto overflow-x-hidden md:overflow-hidden relative bg-black">
      <img src={bgImg} alt="MERCURY Background" aria-hidden="true" className="fixed md:absolute inset-0 w-full h-full object-cover z-0 pointer-events-none" />
      <StarsBackground />
      {/* Mobile Back Button */}
      <button 
        onClick={() => navigate(-1)}
      ...
      <div className="relative md:absolute inset-0 z-10 flex flex-col justify-center items-start px-8 md:px-24 pt-32 pb-20 md:pt-20 md:pb-0 min-h-screen md:min-h-0 pointer-events-none">
        <div ref={contentRef} className="max-w-2xl pointer-events-auto">
          <img src={iconImg} alt="Mercury" className="w-90 h-90 mb-4 md:hidden drop-shadow-lg object-contain" />
          <h1 className="text-6xl md:text-8xl font-bold text-left mb-6 text-gray-300" style={{ textShadow: '0 0 8px #9ca3af, 0 0 24px #6b7280' }}>
            MERCURY
          </h1>

          <div>
            <p className="text-lg mb-6 text-gray-100">Mercury is the smallest planet in our Solar System and the closest to the Sun. It has no atmosphere to retain heat, causing extreme temperature variations.</p>
