...
import bgImg from '../../assets/images/planet-earth-info-pixel.webp';
import iconImg from '../../assets/images/planet-earth-alone-pixel.webp';
...
export default function EarthPage() {
  ...
  const { data, isPending, isError } = useEarth();
  ...
  const buildStats = (d) => [
    ...
    { label: 'Moons', value: d?.moons?.length ?? 1 },
  ];

  return (
    <div ref={containerRef} className="h-screen overflow-y-auto overflow-x-hidden md:overflow-hidden relative bg-black">
      <img src={bgImg} alt="EARTH Background" aria-hidden="true" className="fixed md:absolute inset-0 w-full h-full object-cover z-0 pointer-events-none" />
      <StarsBackground />
      {/* Mobile Back Button */}
      <button 
        onClick={() => navigate(-1)}
      ...
      <div className="relative md:absolute inset-0 z-10 flex flex-col justify-center items-start px-8 md:px-24 pt-32 pb-20 md:pt-20 md:pb-0 min-h-screen md:min-h-0 pointer-events-none">
        <div ref={contentRef} className="max-w-2xl pointer-events-auto">
          ...
          <div>
            <p className="text-lg mb-6 text-blue-100">Earth is the third planet from the Sun and the only known to support life, with liquid water on its surface.</p>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {buildStats(data).map(({ label, value }) => (
                <div key={label} className="p-2">
                  <span className="text-blue-300 font-bold block mb-1">{label}</span>
